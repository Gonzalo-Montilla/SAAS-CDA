...
- [ ] **Turnstile (opcional):** `TURNSTILE_ENABLED`, `TURNSTILE_SECRET_KEY` y **`TURNSTILE_SITE_KEY`** en el **backend** (mismo widget en Cloudflare). El login obtiene la site key con **`GET /api/v1/config/turnstile-public`**, sin depender de `VITE_*` en el build. Para **desactivar** el captcha: `TURNSTILE_ENABLED=False` y reiniciar el backend (sin rebuild del front). En el widget, **Hostnames** debe incluir cada URL donde se abre “Crear CDA”. No bloquear `https://challenges.cloudflare.com` (CSP/WAF).
...
- [ ] Turnstile: preferible **solo variables en el backend** (`TURNSTILE_*`). Las variables **`VITE_TURNSTILE_*`** son solo respaldo si el API no responde; pueden quedar en `false` / vacías.
