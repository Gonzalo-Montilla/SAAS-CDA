...
export async function generarPDFCierreCaja(datos: DatosCierrePDF) {
  ...
  doc.text(formatCOP(datos.resumen.monto_inicial), pageWidth - leftMargin, y - 5, { align: 'right' });
  
  y = addText(`Total Ingresos:`, leftMargin, y, { fontSize: 8 });
  doc.text(formatCOP(datos.resumen.total_ingresos), pageWidth - leftMargin, y - 5, { align: 'right' });
  
  y = addText(`Total Egresos:`, leftMargin, y, { fontSize: 8 });
  doc.text(formatCOP(datos.resumen.total_egresos), pageWidth - leftMargin, y - 5, { align: 'right' });
  
  doc.setFontSize(9);
  y = addText(`Saldo Esperado:`, leftMargin, y, { fontSize: 9, bold: true });
  doc.setFont('helvetica', 'bold');
  doc.text(formatCOP(datos.resumen.saldo_esperado), pageWidth - leftMargin, y - 5, { align: 'right' });
  doc.setFont('helvetica', 'normal');
  y += 1;
  ...
  doc.text(formatCOP(datos.resumen.total_rtm), pageWidth - leftMargin, y - 5, { align: 'right' });
  
  y = addText(`Comision SOAT:`, leftMargin, y, { fontSize: 8 });
  doc.text(formatCOP(datos.resumen.total_comision_soat), pageWidth - leftMargin, y - 5, { align: 'right' });
  y += 1;
  ...
  doc.text(formatCOP(datos.resumen.efectivo), midPoint - 5, yLeft - 5, { align: 'right' });
  
  yLeft = addText(`T. Debito:`, leftMargin, yLeft, { fontSize: 7 });
  doc.text(formatCOP(datos.resumen.tarjeta_debito), midPoint - 5, yLeft - 5, { align: 'right' });
  
  yLeft = addText(`T. Credito:`, leftMargin, yLeft, { fontSize: 7 });
  doc.text(formatCOP(datos.resumen.tarjeta_credito), midPoint - 5, yLeft - 5, { align: 'right' });
  
  // Columna derecha
  let yRight = y;
  yRight = addText(`Transfer:`, midPoint + 5, yRight, { fontSize: 7 });
  doc.text(formatCOP(datos.resumen.transferencia), pageWidth - leftMargin, yRight - 5, { align: 'right' });
  
  yRight = addText(`CrediSmart:`, midPoint + 5, yRight, { fontSize: 7 });
  doc.text(formatCOP(datos.resumen.credismart), pageWidth - leftMargin, yRight - 5, { align: 'right' });
  ...
  if (datos.egresos.length > 0) {
    ...
    datos.egresos.slice(0, 5).forEach((egreso) => { // Máximo 5 gastos en PDF
      ...
      doc.text(`-${formatCOP(Math.abs(egreso.monto))}`, pageWidth - leftMargin, y - 5, { align: 'right' });
    });
  ...
  doc.text(formatCOP(datos.montoFisico), pageWidth - leftMargin, y - 5, { align: 'right' });
  
  doc.setFontSize(9);
  y = addText(`Diferencia:`, leftMargin, y, { fontSize: 9, bold: true });
  doc.setFont('helvetica', 'bold');
  const diferenciaText = datos.diferencia === 0 
    ? `${formatCOP(datos.diferencia)} - CUADRADA` 
    : `${datos.diferencia > 0 ? '+' : ''}${formatCOP(datos.diferencia)}`;
