"""
Aplica la migración baseline multitenant (Fase 1).
"""
import psycopg2
...
def apply_migration():
    ...
    try:
        with open("migrations/add_tenant_baseline.sql", "r", encoding="utf-8") as f:
            sql = f.read()

        print("Aplicando migración baseline multitenant...")
        cursor = conn.cursor()
        cursor.execute(sql)
        conn.commit()
        cursor.close()
