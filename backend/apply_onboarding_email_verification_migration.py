"""
Aplica la migración de verificación de correo para onboarding.
"""
import psycopg2
...
def apply_migration():
    ...
    try:
        with open("migrations/add_onboarding_email_verification.sql", "r", encoding="utf-8") as f:
            sql = f.read()

        print("Aplicando migración de verificación de email onboarding...")
        cursor = conn.cursor()
        cursor.execute(sql)
        conn.commit()
        cursor.close()
