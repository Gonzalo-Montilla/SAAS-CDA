"""
Aplica la migración de scope multitenant para el dominio (Fase 2).
"""
import psycopg2
...
def apply_migration():
    ...
    try:
        with open("migrations/add_tenant_scope_domain.sql", "r", encoding="utf-8") as f:
            sql = f.read()

        print("Aplicando migración de scope multitenant (fase 2)...")
        cursor = conn.cursor()
        cursor.execute(sql)
        conn.commit()
        cursor.close()
