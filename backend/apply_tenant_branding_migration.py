"""
Aplica la migración de branding por tenant (Fase 3-B).
"""
import psycopg2
...
def apply_migration():
    ...
    try:
        with open("migrations/add_tenant_branding.sql", "r", encoding="utf-8") as f:
            sql = f.read()

        print("Aplicando migración de branding por tenant...")
        cursor = conn.cursor()
        cursor.execute(sql)
        conn.commit()
        cursor.close()
