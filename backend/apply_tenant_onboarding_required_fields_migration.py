"""
Aplica la migración de campos obligatorios para onboarding de CDA.
"""
import psycopg2
...
def apply_migration():
    ...
    try:
        with open("migrations/add_tenant_onboarding_required_fields.sql", "r", encoding="utf-8") as f:
            sql = f.read()

        print("Aplicando migración de campos obligatorios onboarding...")
        cursor = conn.cursor()
        cursor.execute(sql)
        conn.commit()
        cursor.close()
