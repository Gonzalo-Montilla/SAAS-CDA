"""
Aplica la migración de campos de facturación de planes SaaS por tenant.
"""
import psycopg2
...
def apply_migration():
    ...
    try:
        with open("migrations/add_tenant_billing_plan_fields.sql", "r", encoding="utf-8") as f:
            sql = f.read()

        print("Aplicando migración de campos de plan/facturación SaaS...")
        cursor = conn.cursor()
        cursor.execute(sql)
        conn.commit()
        cursor.close()
