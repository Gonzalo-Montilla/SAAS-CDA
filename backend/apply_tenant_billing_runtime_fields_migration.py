"""
Aplica la migración de campos operativos de facturación SaaS por tenant.
"""
import psycopg2
...
def apply_migration():
    ...
    try:
        with open("migrations/add_tenant_billing_runtime_fields.sql", "r", encoding="utf-8") as f:
            sql = f.read()

        print("Aplicando migración de campos operativos de facturación...")
        cursor = conn.cursor()
        cursor.execute(sql)
        conn.commit()
        cursor.close()
