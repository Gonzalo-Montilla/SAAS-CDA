"""
Aplica la migración de auth global SaaS (Fase 3).
"""
import psycopg2
...
def apply_migration():
    ...
    try:
        with open("migrations/add_saas_global_auth.sql", "r", encoding="utf-8") as f:
            sql = f.read()

        print("Aplicando migración auth global SaaS (fase 3)...")
        cursor = conn.cursor()
        cursor.execute(sql)
        conn.commit()
        cursor.close()
